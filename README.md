...
## 2026-07-14 更新
- 展示站首次打开时自动使用设备当天真实日期。
- 资产控制台的“表格全览”支持批量新增、按 ID 导入覆盖、导出选中和导出全部。
- “地点库”支持批量新增、按 ID 导入覆盖和导出全部。
- 导入支持 `.xlsx`、`.xls`、`.csv`、`.tsv`、`.txt`；批量新增只接受带表头 TSV。


## Object 首页视觉权重（2026-07-19）

Object 首页使用 1–5 的视觉权重生成跨设备海报式构图：
...
兼容旧数据库值：A→5、B→4、C→3、D→2、空值→1。后台界面、导出和导入均显示 1–5；底层继续兼容原有 Supabase 文本约束。Object 页不再支持手动拖动物件，同一日期和设备下构图保持稳定。
